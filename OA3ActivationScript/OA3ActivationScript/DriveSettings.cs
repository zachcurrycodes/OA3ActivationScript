...
namespace OA3ActivationScript
{
  class DriveSettings
  {
    ...
    public int MapNetworkDrive(string sDriveLetter, string sNetworkPath, string sUserName = null, string sPassword = null)
    {
      ...
      return WNetAddConnection2(ref oNetworkResource, sPassword, sUserName, 0);
    }

    public static int DisconnectNetworkDrive(string sDriveLetter, bool bForceDisconnect)
    {
      if(sDriveLetter[sDriveLetter.Length - 1] == ":"[0])
      {
        sDriveLetter = sDriveLetter[0].ToString();
      }
      if (bForceDisconnect)
      {
